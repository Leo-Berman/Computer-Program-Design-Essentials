...
char* randomstring(int a) {
  // allocates memory for the stirng
  //
  char* arr = (char*)malloc(sizeof(char)*a);

  // loops through creating a string of a length
  //
  for(int i = 0; i<a;i++) {

    // generates a random character
    //
    arr[i]=(int)(100*drand48());

    // makes sure that a null character is not placed in the middle of the string
    //
    if(arr[i]=='\0') {
      i--;
    }
